...
//                val filteredText = newText.filter { it.isLetterOrDigit() }.take(12)
                        ...
                        Text(
                            text = "MASUKKAN ULANG PASSWORD BARU",
                            fontWeight = FontWeight.ExtraBold,
                        )
                        TextField(
                            colors = TextFieldDefaults.colors(
                            ...
                            onValueChange = { newText ->
                                // Filter to allow only alphanumeric characters and limit to 12 characters
//                val filteredText = newText.filter { it.isLetterOrDigit() }.take(12)
                                passBaruConfirm = /*filteredText*/newText
                            },
                            keyboardOptions = KeyboardOptions(
                                keyboardType = KeyboardType.Ascii
                            ),
                            trailingIcon = {
                                IconButton(onClick = {
                                    isPassBaruConfirmHidden = !isPassBaruConfirmHidden
                                }) {
                                    val visibilityIcon =
                    ...
                    Column(
                        ...
                        Text(
                            text = buildAnnotatedString {
                                append("1. Password terdiri dari ")
                                withStyle(style = SpanStyle(fontWeight = FontWeight.Bold)) {
                                    append("8-12 karakter\n")
                                }
                                append("2. Terdiri dari kombinasi ")
                                withStyle(style = SpanStyle(fontWeight = FontWeight.Bold)) {
                                    append("huruf dan angka\n")
                                }
                                append("3. Kombinasi huruf besar dan kecil diperbolehkan")
                            },
                            fontSize = 12.sp
                        )
                    ...
                    Button(
                        ...
                        onClick = {
                            profileNavController.navigate("${FeatureRouteScreens.Security2.route}/${ProfileRouteScreens.UbahPass.route}")
//                            profileNavController.popBackStack()
                        }
                    ) {
                        Text(
